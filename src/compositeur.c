...
int main(int argc, char *argv[])
{
	...
	struct memPartage partages[4];
	struct memPartageHeader *memHeaders[4];
	...
	while (totalFluxs != nb_arg)
	{
		for (size_t i = 0; i < nb_arg; i++)
		{
			if (flagsFluxs[i] != 1) {
				...
				if (attenteLecteurAsync(&partages[i]))
				{
	...
	{
		initMemoirePartageeLecteur(flux[i], &partages[i]);
		pthread_mutex_unlock(&partages[i].header->mutex);
	}
